  ...
  setup() {
    ...
    async function fetchUserFollowers() {
      try {
        ...
        followersCountAccepted.value = await followers.getUserFollowingCountAccepted(
          route.params.id
        )
        followingCountAccepted.value = await followers.getUserFollowersCountAccepted(
          route.params.id
        )
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (Number(route.params.id) !== authStore.user.id) {
          userFollowState.value = await followers.getUserFollowState(
            authStore.user.id,
            route.params.id
          )
        }
