<template>
    <ul class="list-group list-group-flush">
        <li class="list-group-item" v-for="(notification, idx) in notificationsWithPagination" :key="notification.id"
            ...
            <NewActivityNotificationComponent :showDropdown="true" :notification="notification"
                v-if="notification.type === 1" @notificationRead="markNotificationAsRead" />
            <NewActivityDuplicateStartTimeNotificationComponent :notification="notification" :showDropdown="true"
                v-else-if="notification.type === 2" @notificationRead="markNotificationAsRead" />
            <NewFollowerRequestNotificationComponent :notification="notification" :showDropdown="true"
                v-else-if="notification.type === 11" @notificationRead="markNotificationAsRead" />
            <NewAcceptedRequestNotificationComponent :notification="notification" :showDropdown="true"
                v-else-if="notification.type === 12" @notificationRead="markNotificationAsRead" />
        </li>
