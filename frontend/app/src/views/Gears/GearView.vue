    ...
    <div class="col">
      ...
      <div v-else class="bg-body-tertiary p-3 rounded shadow-sm">
        ...
        <NoItemsFoundComponent
          v-if="!gearActivities || (gearActivities && gearActivities.length == 0)"
        />
        <div v-else>
          <ul
            class="list-group list-group-flush"
            v-for="activity in gearActivities"
            :key="activity.id"
            :activity="activity"
          >
            <li class="vstack list-group-item d-flex justify-content-between bg-body-tertiary ps-0">
              ...
              <span
                ><strong>{{ $t('gearView.labelDate') }}:</strong>
                {{ formatDateMed(activity.start_time) }} @
