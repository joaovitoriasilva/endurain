<template>
    ...
    <div v-else-if="summaryData" class="bg-body-tertiary border-0 rounded p-3">
		...
		<div v-if="typeBreakdownData && !selectedActivityType" >
...
const updateColumnVisibility = () => {
	...
	if (width < screenXs) {
		// Below 480px (or your chosen smallest breakpoint)
...
const periodInputId = computed(() => {
	switch (selectedViewType.value) {
		case "week":
			return "periodPickerWeek";
		case "month":
			return "periodPickerMonth";
		case "year":
			return "periodPickerYear";
		default:
			return "periodPicker";
	}
});
...
const periodLabel = computed(() => {
	switch (selectedViewType.value) {
		case "week":
			return t("summaryView.labelSelectWeek");
		case "month":
			return t("summaryView.labelSelectMonth");
		case "year":
			return t("summaryView.labelSelectYear");
		case "lifetime":
			return "";
		default:
...
const summaryPeriodText = computed(() => {
	...
	if (!summaryData.value && !isLoading.value)
		return t("summaryView.labelSelectPeriod");
	if (isLoading.value) return t("generalItems.labelLoading");
...
const breakdownHeader = computed(() => {
	switch (selectedViewType.value) {
		case "week":
			return t("summaryView.colDay");
		case "month":
			return t("summaryView.colWeekNum");
		case "year":
			return t("summaryView.colMonth");
		case "lifetime":
			return t("summaryView.colYear");
		default:
			return "Period";
	}
});
...
const fetchSummaryData = async () => {
	...
	try {
		...
		if (selectedViewType.value !== "lifetime") {
			...
			totalActivities.value = 0;
			currentPage.value = 1;
			isLoadingActivities.value = false; // Ensure loading state is false
			errorActivities.value = null; // Clear any previous activity errors
		}
	} catch (err) {
		console.error("Error fetching summary:", err);
		errorSummary.value =
			err.message || err.response?.data?.detail || t("generalItems.labelError");
	} finally {
...
const fetchActivitiesForPeriod = async (page = currentPage.value) => {
	if (!authStore.user?.id || selectedViewType.value === "lifetime") {
		// Do not fetch for lifetime
		activities.value = [];
		totalActivities.value = 0;
...
function handlePageChange(newPage) {
	if (activitiesSectionRef.value) {
		storedScrollPosition =
			activitiesSectionRef.value.getBoundingClientRect().top;
	}
...
function triggerDataFetch() {
	...
	if (shouldFetchSummary) {
		...
		console.warn(
			"Skipping summary fetch due to invalid period selection for view type:",
			selectedViewType.value,
		);
		summaryData.value = null;
		typeBreakdownData.value = null;
...
const calculatedTotalPages = computed(() => {
	return totalActivities.value > 0
		? Math.ceil(totalActivities.value / activitiesPerPage.value)
		: 1;
});
