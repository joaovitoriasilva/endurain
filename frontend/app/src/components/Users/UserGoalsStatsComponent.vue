        ...
        <li class="list-group-item d-flex justify-content-between px-0 bg-body-tertiary">
            <div class="flex-grow-1">
                <div>
                    ...
                    <span v-if="goal.goal_type === 5">{{ formatDuration(t, goal.total_duration) }}{{
                        $t("generalItems.ofWithSpaces") }}{{
                            formatDuration(t, goal.goal_duration) }}</span>
                </div>
                <div class="progress" role="progressbar" aria-label="Goal tracking done vs target"
                    :aria-valuenow="goal.percentage_completed" aria-valuemin="0" aria-valuemax="100">
                    <div class="progress-bar" :style="{ width: goal.percentage_completed + '%' }">{{
                        goal.percentage_completed }}%</div>
                </div>
            </div>
        </li>
    </ul>
