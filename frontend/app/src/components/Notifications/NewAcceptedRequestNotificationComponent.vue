<template>
    <router-link class="dropdown-item link-body-emphasis text-wrap"
        ...
        <span><b>{{ $t('newAcceptedRequestNotificationComponent.newAcceptedRequestTitle') }}</b></span>
        <br>
        <span class="fw-lighter">
            {{ notification.options['user_name'] }} - @{{ notification.options['user_username'] }}{{ $t('newAcceptedRequestNotificationComponent.newAcceptedRequestSubTitle') }}
        </span>
    </router-link>
</template>
