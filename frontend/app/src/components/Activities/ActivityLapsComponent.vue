    ...
    <table class="table table-sm table-borderless" style="--bs-table-bg: var(--bs-gray-850)">
      <thead>
        <tr>
          ...
          <th
            scope="col"
            style="width: 10%"
            v-if="!activityTypeIsSwimming(activity) && activity.activity_type !== 13"
          >
      ...
      <tbody>
        <tr v-for="(lap, index) in normalizedLaps" :key="index">
          ...
          <td v-if="!activityTypeIsSwimming(activity) && activity.activity_type !== 13">
    ...
    const normalizedLaps = computed(() => {
      ...
      return lapsWithRest.map((lap) => {
        ...
        const formattedPace = formatPace(
          props.activity,
          authStore.user.units,
          lap,
          false,
          lap.swimIsRest
        )
        const formattedPaceFull = formatPace(
          props.activity,
          authStore.user.units,
          lap,
          true,
