<template>
	<div class="col">
		<div class="bg-body-tertiary rounded p-3 shadow-sm">
				...
				<li class="list-group-item d-flex justify-content-between bg-body-tertiary px-0 pt-0">
					...
					<div class="d-flex align-items-center">
						...
						<a href="#" class="btn btn-primary" role="button" data-bs-toggle="modal"
							data-bs-target="#retrieveStravaClientIdModal" v-if="authStore.user.is_strava_linked == 0">{{
							...
							<ul class="dropdown-menu">
								...
								<li>
									...
									<a href="#" class="dropdown-item" @click="submitRetrieveStravaGear">{{
										$t("settingsIntegrationsZone.buttonRetrieveGear") }}</a>
								</li>
								<li>
				...
				<li class="list-group-item d-flex justify-content-between bg-body-tertiary px-0">
					...
					<div class="d-flex align-items-center">
						...
						<a href="#" class="btn btn-primary" role="button" @click="submitBulkImport">{{
							$t("settingsIntegrationsZone.buttonBulkImport") }}</a>
					</div>
				</li>
				<!-- Garmin Connect zone -->
				<li class="list-group-item d-flex justify-content-between bg-body-tertiary px-0 pb-0">
					<div class="d-flex align-items-center">
						...
						<img src="/src/assets/garminconnect/Garmin_Connect_app_1024x1024-02.png"
							alt="Garmin Connect logo" height="32" />
						<div class="ms-3">
							<div class="fw-bold">
					...
					<div class="d-flex align-items-center">
						<!-- connect button -->
						<a href="#" class="btn btn-primary" v-if="authStore.user.is_garminconnect_linked == 0"
							data-bs-toggle="modal" data-bs-target="#garminConnectAuthModal">{{
								$t("settingsIntegrationsZone.buttonConnect") }}</a>
			...
			<ModalComponentNumberAndStringInput modalId="retrieveStravaClientIdModal"
				...
				:actionButtonType="`success`" :actionButtonText="t('settingsIntegrationsZone.buttonConnect')"
				@fieldsToEmitAction="submitConnectStrava" />

			<!-- modal retrieve Strava activities by days -->
			<ModalComponentNumberInput modalId="retrieveStravaActivitiesByDaysModal"
				...
				:actionButtonType="`success`" :actionButtonText="t('settingsIntegrationsZone.modalRetrieveButton')"
				@numberToEmitAction="submitRetrieveStravaActivities" />

			<!-- modal unlink Strava -->
			<ModalComponent modalId="unlinkStravaModal" :title="t('settingsIntegrationsZone.modalUnlinkStravaTitle')"
				:body="`${t('settingsIntegrationsZone.modalUnlinkStravaBody')}`" :actionButtonType="`danger`"
			...
			<ModalComponentNumberInput modalId="retrieveGarminConnectActivitiesByDaysModal"
				:title="t('settingsIntegrationsZone.modalRetrieveActivitiesByDaysTitle')"
				:numberFieldLabel="`${t('settingsIntegrationsZone.modalRetrieveActivitiesByDaysLabel')}`"
				:actionButtonType="`success`" :actionButtonText="t('settingsIntegrationsZone.modalRetrieveButton')"
			...
			<ModalComponentNumberInput modalId="retrieveGarminConnectHealthDataByDaysModal"
			...
			<ModalComponent modalId="unlinkGarminConnectModal"
				:title="t('settingsIntegrationsZone.modalUnlinkGarminConnectTitle')"
				:body="`${t('settingsIntegrationsZone.modalUnlinkGarminConnectBody')}`" :actionButtonType="`danger`"
...
async function submitConnectStrava(stravaClient) {
	...
	try {
		...
		try {
			...
			push.error(`${t("settingsIntegrationsZone.errorMessageUnableToUnsetStravaClientSettings")} - ${error}`);
		}
	} finally {
...
async function submitRetrieveStravaActivities(daysToRetrieveStrava) {
	try {
		...
		push.error(`${t("settingsIntegrationsZone.errorMessageUnableToGetStravaActivities")} - ${error}`);
	}
}
...
async function submitRetrieveGarminConnectActivitiesDays(days) {
	try {
		...
		push.error(`${t("settingsIntegrationsZone.errorMessageUnableToGetGarminConnectActivitiesDays")} - ${error}`);
	}
}
...
async function submitRetrieveGarminConnectActivitiesDataRange(dateRange) {
	try {
		...
		push.error(`${t("settingsIntegrationsZone.errorMessageUnableToGetGarminConnectActivitiesDataRange")} - ${error}`);
	}
}
...
async function submitRetrieveGarminConnectGear() {
	try {
		...
		push.error(`${t("settingsIntegrationsZone.errorMessageUnableToGetGarminConnectGear")} - ${error}`);
	}
}
...
async function submitRetrieveGarminConnectHealthDataDataRange(dateRange) {
	try {
		...
		push.error(`${t("settingsIntegrationsZone.errorMessageUnableToGetGarminConnectHealthDataDateRange")} - ${error}`);
	}
}
...
async function submitRetrieveGarminConnectHealthDataDays(days) {
	try {
		...
		push.error(`${t("settingsIntegrationsZone.errorMessageUnableToGetGarminConnectHealthDataDays")} - ${error}`);
	}
}
...
async function buttonGarminConnectUnlink() {
	...
	try {
		...
		notification.reject(`${t("settingsIntegrationsZone.errorMessageUnableToUnlinkGarminConnect")} - ${error}`);
	}
}
</script>
