		...
		<div class="bg-body-tertiary rounded p-3 shadow-sm">
			...
			<div v-else>
				<div v-if="!mfaEnabled">
					...
					<button type="button" class="btn btn-primary" @click="setupMFA" :disabled="mfaSetupLoading">
						<span v-if="mfaSetupLoading" class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>
						{{ $t("settingsSecurityZone.enableMFAButton") }}
					</button>
				</div>
				<div v-else>
					<p>{{ $t("settingsSecurityZone.mfaEnabledDescription") }}</p>
					<button type="button" class="btn btn-danger" @click="showDisableMFAModal" :disabled="mfaDisableLoading">
			...
			<!-- MFA Setup Modal -->
			<div class="modal fade" id="mfaSetupModal" tabindex="-1" aria-labelledby="mfaSetupModalLabel" aria-hidden="true" ref="mfaSetupModal">
				<div class="modal-dialog modal-dialog-centered">
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title" id="mfaSetupModalLabel">{{ $t("settingsSecurityZone.mfaSetupModalTitle") }}</h5>
						...
						<div class="modal-body">
							<div v-if="qrCodeData">
								...
								<p><strong>{{ $t("settingsSecurityZone.mfaSecretLabel") }}:</strong> <code>{{ mfaSecret }}</code></p>
								<form @submit.prevent="enableMFA">
									<label for="mfaVerificationCode"><b>* {{ $t("settingsSecurityZone.mfaVerificationCodeLabel") }}</b></label>
									<input type="text" class="form-control" id="mfaVerificationCode" v-model="mfaVerificationCode" :placeholder="$t('settingsSecurityZone.mfaVerificationCodePlaceholder')" required>
									<p class="mt-2">* {{ $t("generalItems.requiredField") }}</p>
									<div class="d-flex justify-content-end">
										<button type="button" class="btn btn-secondary me-2" data-bs-dismiss="modal">{{ $t("generalItems.cancel") }}</button>
										<button type="submit" class="btn btn-success" :disabled="!mfaVerificationCode || mfaEnableLoading">
											<span v-if="mfaEnableLoading" class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>
											{{ $t("settingsSecurityZone.enableMFAButton") }}
										</button>
									</div>
								</form>
			...
			<!-- MFA Disable Modal -->
			<div class="modal fade" id="mfaDisableModal" tabindex="-1" aria-labelledby="mfaDisableModalLabel" aria-hidden="true" ref="mfaDisableModal">
				<div class="modal-dialog modal-dialog-centered">
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title" id="mfaDisableModalLabel">{{ $t("settingsSecurityZone.mfaDisableModalTitle") }}</h5>
							<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
						</div>
						<div class="modal-body">
							<p>{{ $t("settingsSecurityZone.mfaDisableConfirmation") }}</p>
							<form @submit.prevent="disableMFA">
								<label for="mfaDisableCode"><b>* {{ $t("settingsSecurityZone.mfaVerificationCodeLabel") }}</b></label>
								<input type="text" class="form-control" id="mfaDisableCode" v-model="mfaDisableCode" :placeholder="$t('settingsSecurityZone.mfaVerificationCodePlaceholder')" required>
								<p class="mt-2">* {{ $t("generalItems.requiredField") }}</p>
								<div class="d-flex justify-content-end">
									<button type="button" class="btn btn-secondary me-2" data-bs-dismiss="modal">{{ $t("generalItems.cancel") }}</button>
									<button type="submit" class="btn btn-danger" :disabled="!mfaDisableCode || mfaDisableLoading">
										<span v-if="mfaDisableLoading" class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>
										{{ $t("settingsSecurityZone.disableMFAButton") }}
									</button>
								</div>
							</form>
...
import { mfa } from "@/services/mfaService";
// Import Notivue push
	...
	setup() {
		...
		async function loadMFAStatus() {
			try {
				...
				const status = await mfa.getMFAStatus();
		...
		async function setupMFA() {
			try {
				...
				const setupData = await mfa.setupMFA();
		...
		async function enableMFA() {
			try {
				...
				await mfa.enableMFA({ mfa_code: mfaVerificationCode.value });
		...
		async function disableMFA() {
			try {
				...
				await mfa.disableMFA({ mfa_code: mfaDisableCode.value });
