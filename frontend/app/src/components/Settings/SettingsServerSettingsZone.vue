    ...
    <form class="bg-body-tertiary rounded p-3 shadow-sm">
      ...
      <div class="row">
        <div class="col">
          ...
          <div
            class="modal fade"
            id="addLoginPhotoModal"
            tabindex="-1"
            aria-labelledby="addLoginPhotoModalLabel"
            aria-hidden="true"
          >
              ...
              <div class="modal-content">
                ...
                <form @submit.prevent="submitUploadFileForm">
                  <div class="modal-body">
                    <!-- date fields -->
                    <label for="loginPhotoAdd"
                      ><b
                        >* {{ $t('settingsServerSettingsZoneComponent.logonPhotoAddLabel') }}</b
                      ></label
                    >
                    <br />
                    <span>{{
                      $t('settingsServerSettingsZoneComponent.logonPhotoAddDetailsLabel')
                    }}</span>
                    <input
  ...
  setup() {
    ...
    const submitUploadFileForm = async () => {
      ...
      const notification = push.promise(
        t('settingsServerSettingsZoneComponent.processingPhotoUpload')
      )
    ...
    const submitDeleteLoginPhoto = async () => {
      // Set the loading message
      const notification = push.promise(
        t('settingsServerSettingsZoneComponent.processingPhotoDelete')
      )
    ...
    // watchers
    watch(
      [units, publicShareableLinks, publicShareableLinksUserInfo],
      async () => {
        await updateServerSettings()
      },
      { immediate: false }
